...
/**
 *  快速创建标签，会根据文字和字体自动计算尺寸，但是需要注意的是需要给定X,Y
 *
 *  @param text      文字
 *  @param font      字体
 *  @param textColor 文本颜色
...
+(UILabel *)labelWithText:(NSString *)text
                     font:(UIFont *)font
                textColor:(UIColor *)textColor{
    return [self labelWithText:text font:font maxW:MAXFLOAT textColor:textColor];
}
...
#pragma mark - 特效
//MARK-感谢SystemOuter的参照源码ZYPZPro
/**
 *  创建一个Label标签
 *
 ...
 *  @return 返回对应Label
 */
+(UILabel *)label_AllocAttributedString:(USLabelBasicSetBlock)basicSet withText:(NSString *)text attributedMode:(NSArray *)attributedArray{
    //常规创建
    UILabel * lab_Temp = [[UILabel alloc] init];
    if (basicSet) {
        basicSet(lab_Temp);
    ...
    for (NSValue * value_Temp in attributedArray) {
        //取出NSValue中结构体
        USAttributedMode attriM_Temp = ValueToPZAttributedMode(value_Temp);
        //赋值给格式字符串
